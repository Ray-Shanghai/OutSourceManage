	...
	//获取类别数据
	 $.ajax( {
        ...
        success : function(result) {
        	$(".add1").eq(index).find("select[name='productTypeId']").html("");
            if (result.code==0) {
            	$(".add1").eq(index).find("select[name='productTypeId']").append("<option value=''>请选择</option>")
           	 for(var i=0;i<result.rows.length;i++){
           		$(".add1").eq(index).find("select[name='productTypeId']").append(new Option(result.rows[i].productTypeName,result.rows[i].productTypeId));
           	}
           }
       },error:function(){}
	 });
	 ...
	 $.ajax( {
	        ...
	        async: false,
	        success : function(result) {
	        	$(".add1").eq(index).find("select[name='productGradeId']").html("");
	            if (result.code==0) {
	            	$(".add1").eq(index).find("select[name='productGradeId']").append("<option value=''>请选择</option>")
	           	 for(var i=0;i<result.rows.length;i++){
	           		$(".add1").eq(index).find("select[name='productGradeId']").append(new Option(result.rows[i].productGradeName,result.rows[i].productGradeId));
	           	}
	           }
	       },error:function(){}
	 });
